...
import { TOTAL_STORYBOOK_PAGES } from "@/lib/storybook-generation";
...
export default async function OrderPreviewPage({
  ...
  const needsPaidPages =
    paid && order.illustrations.length < TOTAL_STORYBOOK_PAGES;
  const needsPreviewPages = !paid && order.illustrations.length === 0;
      ...
      <IntervalRefresher
        ...
        initialSignature={JSON.stringify(
          illustrationStatusPayload(
            (paid ? order.illustrations : previewIllustrations)
              .filter((page) => page.id)
              .map((page) => ({
                id: page.id,
                status: page.status,
